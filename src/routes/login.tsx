        ...
        <div className="relative glass rounded-3xl border border-white/10 p-8 md:p-10 overflow-hidden">
          ...
          <div className="mt-7 grid grid-cols-2 rounded-xl border border-white/10 bg-white/5 p-1 text-sm">
            {(["login", "signup"] as const).map((m) => (
              <button
                ...
                className={`relative rounded-lg py-2 font-medium transition-colors ${
                  mode === m ? "text-white" : "text-slate-400 hover:text-slate-200"
                }`}
              >
                {mode === m && (
                  <span className="absolute inset-0 rounded-lg bg-gradient-to-r from-violet-600/80 to-cyan-500/60 shadow-[0_0_20px_-4px_rgba(124,58,237,0.6)]" />
                )}
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            <div>
              ...
              <div className="relative mt-1.5">
                ...
                <input
                  ...
                  className="auth-input w-full rounded-xl border border-white/10 bg-white/5 py-3 pl-10 pr-3 text-sm text-slate-100 placeholder:text-slate-500 transition"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative mt-1.5">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="auth-input w-full rounded-xl border border-white/10 bg-white/5 py-3 pl-10 pr-10 text-sm text-slate-100 placeholder:text-slate-500 transition"
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              disabled={loading}
              className="auth-btn group relative flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold text-white transition disabled:opacity-70"
            >
              {loading ? (
